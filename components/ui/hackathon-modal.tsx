...
interface HackathonPopoverProps {
  show?: boolean;
}

export function HackathonPopover({ show = false }: HackathonPopoverProps) {
  const [isDismissed, setIsDismissed] = useState(false);

  const isOpen = show && !isDismissed;
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="hidden sm:block fixed bottom-6 right-6 z-100 w-90 bg-black/50 border border-[#2e2e2e] rounded-lg shadow-2xl overflow-hidden"
        >
